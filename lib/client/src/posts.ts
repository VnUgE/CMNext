 ...
 */
export const createAutoPostApi = ({
  cmsChannelIndexPath,
  channelId,
  urlPrefix,
}: CMNextAutoConfig): AutoPostApi => {
  ...
  const getPostContent = async (post: PostMeta | string, extension = '.html'): Promise<string> => {
    ...
    const itemId = typeof post === 'string' ? post : post.id;
 ...
 * @returns A post api that can be used to get posts from the channel
 */
export const createManualPostApi = ({
  channelRootDir,
  contentDir,
  postIndexPath,
}: PostApiManualConfig): ManualPostApi => {
  //Make sure index file has a leading slash
  const fixedPostIndexPath = startsWith(postIndexPath, '/') ? postIndexPath : `/${postIndexPath}`;
  ...
  const getIndex = async (): Promise<CMNextIndex<PostMeta>> => {
    //Fetch the index file
    const res = await fetch(getItemPath(fixedPostIndexPath));
    return await res.json();
  };

  const getPostContent = async (post: PostMeta | string, extension = '.html'): Promise<string> => {
    //Get the content url
    const contentUrl = `${getItemPath(fixedContentDir)}/${typeof post === 'string' ? post : post.id}${extension}`;
  ...
  const getIndexFilePath = async (): Promise<string> => {
    return getItemPath(fixedPostIndexPath);
  };

  return {
    getIndex,
    channelRootDir,
    contentDir: fixedContentDir,
    postIndexPath: fixedPostIndexPath,
    getPostContent,
    getIndexFilePath,
  };
};
