...
import { find, reduce, take } from 'lodash-es';
...
const channel = computed(() => find(store.channels.all, c => c.id === channelId.value));
const { all: posts, isLoading: arePostsLoading } = store.createPostStore(channelId);
const { all: content, isLoading: isContentLoading } = store.createContentStore(channelId);

const isLoading = computed(() => arePostsLoading.value || isContentLoading.value);
...
const getChannelFeedUrl = () => channel.value?.feed?.url || `https://example.com/feed/${channelId.value}`;
...
const formatDate = (dateString?: number) => {
    if(dateString === undefined || dateString === null) return 'Never'
...
<template>
  <div class="p-6 space-y-6">
    ...
    <div v-else-if="!channel" class="text-center py-12">
      ...
      <p class="text-base-content/50 mb-6">The requested channel could not be found or you don't have access to it.</p>
    ...
    <div v-else>
      ...
      <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div class="flex items-center gap-4">
          <router-link to="/blog/channels" class="btn btn-ghost btn-sm">
            <fa-icon icon="arrow-left" class="mr-2" />
            Back
          </router-link>
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-base-content/70">{{ channel.path || 'No path provided' }}</p>
            </div>
          </div>
        </div>
        <div class="flex gap-2">
          <router-link :to="`/blog/posts/${channelId}`" class="btn btn-primary">
        ...
        <div class="card bg-base-100 shadow">
          <div class="card-body">
            ...
            <div v-if="recentPosts.length === 0" class="text-center py-8">
              ...
              <router-link :to="`/blog/posts/new?channel=${channelId}`" class="btn btn-sm btn-primary mt-2">
                Create First Post
              </router-link>
            </div>
            <div v-else class="space-y-3">
              <div v-for="post in recentPosts" :key="post.id" 
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="text-sm text-base-content/70">{{ formatDate(post.date) }}</div>
                </div>
                <div class="flex items-center gap-2">
                  <div class="badge badge-sm" :class="post.isPublished || true ? 'badge-success' : 'badge-warning'">
                    {{ post.isPublished || true ? 'Published' : 'Draft' }}
                  </div>
                  <router-link :to="`/blog/posts/edit?channel=${channelId}&post=${post.id}`" class="btn btn-xs btn-ghost">
                    <fa-icon icon="edit" />
                  </router-link>
                </div>
              </div>
            ...
            <div class="card-actions justify-end mt-4">
              <router-link :to="`/blog/posts/${channelId}`" class="btn btn-sm btn-outline">
                View All Posts
              </router-link>
            </div>
          </div>
        ...
        <div class="card bg-base-100 shadow">
          <div class="card-body">
            ...
            <div class="grid grid-cols-1 gap-3">
              <router-link :to="`/blog/posts/new?channel=${channelId}`" 
                           class="btn btn-outline btn-lg justify-start gap-3">
                <fa-icon icon="plus" />
                New Post
              </router-link>
              <router-link :to="`/blog/content/${channelId}`" 
                           class="btn btn-outline btn-lg justify-start gap-3">
                <fa-icon icon="folder-open" />
                Manage Content
              </router-link>
              <router-link :to="`/blog/channels/edit?id=${channelId}`" 
                           class="btn btn-outline btn-lg justify-start gap-3">
                <fa-icon icon="cog" />
                Channel Settings
              </router-link>
              <a :href="getChannelFeedUrl()" target="_blank" 
                 class="btn btn-outline btn-lg justify-start gap-3">
                <fa-icon icon="rss" />
                View RSS Feed
              </a>
      ...
      <div class="card bg-base-100 shadow py-2">
        <div class="card-body">
            ...
            <div>
              <div class="space-y-4">
                ...
                <div>
                  ...
                  <div class="text-sm font-mono break-all">{{ getChannelFeedUrl() }}</div>
                </div>
                <div>
          ...
          <div class="card-actions justify-end mt-6">
            <router-link :to="`/blog/channels/edit?id=${channelId}`" class="btn btn-primary">
              <fa-icon icon="cog" class="mr-2" />
              Edit Settings
            </router-link>
          </div>
