...
const { refresh, isLoading } = store.channels

const searchFilter = useArrayFilter(store.channels.all, (channel) => {
...
const deleteChannel = async (channel: any) => {
    const { isCanceled } = await confirm({
        ...
        text: `Are you sure you want to delete the channel "${channel.name}"? This action cannot be undone.`,
        isWarning: true,
    })
    ...
    await apiCall(async ({ toaster }) => {
        await store.channels.delete(channel.id);

        toaster.general.success({
            title: 'Channel Deleted',
            text: `Channel "${channel.name}" has been deleted successfully.`,
        })
    })
}
  ...
  <div class="p-6 space-y-6">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
      ...
      <div class="flex gap-2">
        <button @click="refresh()" class="btn btn-outline" :disabled="isLoading">
          <fa-icon icon="sync" :class="{ 'animate-spin': isLoading }" class="mr-2" />
          Refresh
        </button>
        <router-link to="/blog/channels/new" class="btn btn-primary">
    ...
    <div v-else-if="searchFilter.length === 0" class="text-center py-12">
      ...
      <p class="text-base-content/50 mb-6">{{ searchTerm ? 'Try adjusting your search criteria' : 'Create your first channel to get started' }}</p>
      <router-link to="/blog/channels/new" class="btn btn-primary">
        <fa-icon icon="plus" class="mr-2" />
        Create Channel
      </router-link>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div v-for="channel in searchFilter" :key="channel.id" 
        ...
        <div class="card-body">
          <div class="flex items-start justify-between mb-3">
            ...
            <div class="dropdown dropdown-left ml-auto">
              ...
              <ul tabindex="0" class="dropdown-content menu bg-base-100 rounded-box z-[1] w-52 p-2 shadow">
                ...
                <li>
                  <router-link :to="`/blog/channels/edit?id=${channel.id}`">
                    <fa-icon icon="edit" class="mr-2" />
                    Edit Settings
                  </router-link>
                </li>
                <li>
                  <a @click="togglePin(channel.id)" class="text-primary">
                    <fa-icon :icon="pinnedChannels.isPinned(channel.id) ? 'thumbtack-slash' : 'thumbtack'" class="mr-2" />
                    {{ pinnedChannels.isPinned(channel.id) ? 'Unpin Channel' : 'Pin Channel' }}
                  </a>
                </li>
          ...
          <div class="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <div class="font-semibold"> {{ feedEnabled(channel) ? 'Configured' : 'Not Configured' }}</div>
            </div>
            <div>
          ...
          <div class="card-actions justify-end mt-4">
            <router-link :to="`/blog/posts/${channel.id}`" class="btn btn-sm btn-outline">
              Manage Posts
            </router-link>
            <router-link :to="`/blog/channels/${channel.id}`" class="btn btn-sm btn-primary">
              View Channel
            </router-link>
          </div>
        </div>
