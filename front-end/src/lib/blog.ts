...
import 'pinia'
import { computed, type MaybeRef, type Ref, ref, toRef } from 'vue';
import { type AxiosProgressEvent } from 'axios';
import { ApiConfig, useAxios } from '@vnuge/vnlib.browser';
import { useAsyncState } from '@vueuse/core';
import { find as _find, filter, first } from 'lodash-es'
import {
    usePosts,
    useContent,
    useChannels,
...
export const useCmnextAdmin = (vnlib: ApiConfig, adminBaseUrl: string): BlogAdminState => {
    ...
    : ReactiveBlogStore<T> & TStore => {
        ...
        const single = (id: MaybeRef<string>): Ref<T | undefined> => {
            const _id = toRef(id);
            return computed(() => {
                const filtered = filter(all.value, c => c.id == _id.value);
